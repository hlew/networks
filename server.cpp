...
STATE send_data(Connection *client, uint8_t *packet, int32_t *packet_len, int32_t data_file, int buf_size, int32_t *seq_num, buffer *window) {
	...
	default:
		(*packet_len) = send_buf(buf, window[i].len_read, client, DATA, window[i].seq_num, packet);
...
STATE wait_on_ack(Connection *client, buffer *window) {
	...
	if (client->win_status == OPEN) {
		if (select_call(client->sk_num, 0, 0, SET_NULL) != 1) {
			return (SEND_DATA);
		}
		else {
			// do nothing;
...
//		exit(-1);
//	}
	window[client->base % client->window_size].send_status = NOT_SENT;
